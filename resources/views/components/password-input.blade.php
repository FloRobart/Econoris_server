...
<div>
    <label for="password" class="labelForm">@if ($confirmation) Confirmation du @endif Mot de passe @include('components.asterisque')/></label>
    <div class="relative">
        <input @if ($confirmation) name="password_confirmation" id="password_confirmation" @else name="password" id="password" @endif type="password" minlength="4" maxlength="20" @if ($newPassword) autocomplete="new-password" @else autocomplete="current-password" @endif class="inputForm" placeholder="Entrez votre mot de passe" value="" required>
